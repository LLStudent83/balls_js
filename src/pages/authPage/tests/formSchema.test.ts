...
describe('formSchema', () => {
  ...
  describe('Валидация nickName', () => {
    it('должна вернуть ошибку для nickName меньше 2 символов', () => {
      ...
      if (!result.success) {
        const nickNameError = result.error.issues.find((issue) => issue.path[0] === 'nickName');
        expect(nickNameError).toBeDefined();
        expect(nickNameError?.message).toBe('Имя пользователя должно быть не меньше 2-х символов');
      }
    });
